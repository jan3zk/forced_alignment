...
def get_average_pitch_for_phonemes(audio_path, textgrid_path, output_path=None):
    ...
    for start, end, label in entry_list:
        ...
        phoneme_pitch_vals = [val for val in pitch_values[start_index:end_index] if val != 0]
        phoneme_intensity_vals = [val for val in intensity_values[start_index:end_index] if val > 0]

        if phoneme_pitch_vals:
            avg_pitch = sum(phoneme_pitch_vals) / len(phoneme_pitch_vals)
        else:
            avg_pitch = None

        if phoneme_intensity_vals:
            avg_intensity = sum(phoneme_intensity_vals) / len(phoneme_intensity_vals)
        else:
            avg_intensity = None

        results[label] = (avg_pitch, avg_intensity)

    # Save to txt file
    if output_path:
        with open(output_path, 'w') as file:
            file.write("Phoneme\tAverage Pitch\tAverage Intensity\n")
            for phoneme, (avg_pitch, avg_intensity) in results.items():
                file.write(f"{phoneme}\t{avg_pitch if avg_pitch else 'N/A'}\t{avg_intensity if avg_intensity else 'N/A'}\n")
